...
using FnLspHook = jobject (*)(JNIEnv *env, jobject target_method, jobject hooker_object, jobject callback_method);
...
static bool ResolveLsplantSymbols(void *handle) {
    ...
    if (!g_fn_init) {
        LOGE("missing symbol: lsp_native_init, err=%s", e1 ? e1 : "null");
        return false;
    }
    ...
    if (!g_fn_hook) {
        LOGE("missing symbol: lsp_native_hook, err=%s", e2 ? e2 : "null");
        return false;
    }
    ...
    if (!g_fn_unhook) {
        LOGE("missing symbol: lsp_native_unhook, err=%s", e3 ? e3 : "null");
        return false;
    }

    dlerror();
    g_fn_is_hooked = reinterpret_cast<FnLspIsHooked>(dlsym(handle, "lsp_native_is_hooked"));
    const char *e4 = dlerror();
    if (!g_fn_is_hooked) {
        LOGE("missing symbol: lsp_native_is_hooked, err=%s", e4 ? e4 : "null");
        return false;
    }

    dlerror();
    g_fn_deoptimize = reinterpret_cast<FnLspDeoptimize>(dlsym(handle, "lsp_native_deoptimize"));
    const char *e5 = dlerror();
    if (!g_fn_deoptimize) {
        LOGE("missing symbol: lsp_native_deoptimize, err=%s", e5 ? e5 : "null");
        return false;
    }
    ...
    const char *e6 = dlerror();
    if (!g_fn_make_class_inheritable) {
        LOGE("missing symbol: lsp_native_make_class_inheritable, err=%s", e6 ? e6 : "null");
        return false;
    }
...
Java_de_robv_android_xposed_XposedBridge_nativeInitLsplant(
        JNIEnv *env, jclass, jstring lsplantPath_) {
    if (g_lsplant_inited) {
        LOGE("nativeInitLsplant: already inited");
        return JNI_TRUE;
    }

    if (lsplantPath_ == nullptr) {
        LOGE("nativeInitLsplant: path is null");
        return JNI_FALSE;
    }

    const char *lsplant_path = env->GetStringUTFChars(lsplantPath_, nullptr);
    if (lsplant_path == nullptr) {
        LOGE("nativeInitLsplant: GetStringUTFChars failed");
        return JNI_FALSE;
    }

    LOGE("nativeInitLsplant: input path=%s", lsplant_path);

    int api_level = android_get_device_api_level();
    LOGE("nativeInitLsplant: api_level=%d", api_level);
    if (api_level <= 0) {
        env->ReleaseStringUTFChars(lsplantPath_, lsplant_path);
        LOGE("nativeInitLsplant: invalid api_level");
        return JNI_FALSE;
    }
    ...
    if (!g_lsplant_handle) {
        ...
        LOGE("nativeInitLsplant: dlopen failed, err=%s", err ? err : "null");
        env->ReleaseStringUTFChars(lsplantPath_, lsplant_path);
        return JNI_FALSE;
    }
    LOGE("nativeInitLsplant: dlopen ok");

    env->ReleaseStringUTFChars(lsplantPath_, lsplant_path);

    if (!ResolveLsplantSymbols(g_lsplant_handle)) {
        LOGE("nativeInitLsplant: ResolveLsplantSymbols failed");
        dlclose(g_lsplant_handle);
        g_lsplant_handle = nullptr;
        return JNI_FALSE;
    }
    LOGE("nativeInitLsplant: ResolveLsplantSymbols ok");
    ...
    LOGE("nativeInitLsplant: lsp_native_init ret=%d", res ? 1 : 0);

    if (env->ExceptionCheck()) {
        LOGE("nativeInitLsplant: exception after lsp_native_init");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }

    if (!res) {
        LOGE("nativeInitLsplant: lsp_native_init failed");
        dlclose(g_lsplant_handle);
        g_lsplant_handle = nullptr;
        return JNI_FALSE;
    }
    ...
    LOGE("nativeInitLsplant: LoadInvokeConstructorCache ret=%d", res ? 1 : 0);

    if (env->ExceptionCheck()) {
        LOGE("nativeInitLsplant: exception after LoadInvokeConstructorCache");
        env->ExceptionDescribe();
        env->ExceptionClear();
    }

    if (!res) {
        LOGE("nativeInitLsplant: LoadInvokeConstructorCache failed");
        dlclose(g_lsplant_handle);
        g_lsplant_handle = nullptr;
        return JNI_FALSE;
    }
    ...
    LOGE("nativeInitLsplant: success");
    return JNI_TRUE;
}
